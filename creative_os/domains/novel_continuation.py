...
def build_next_chapter(project_root: str | Path) -> tuple[ContinuationTask, CompiledContext]:
    ...
    baseline = _read_json(root / ".creative_os" / "import" / "baseline.json")
    continuation = _continuation_task(last_number, last_text, baseline)
    task = Task(
    ...
    compiled = ContextCompiler().compile(
        CompileRequest(
            ...
            state=state,
            knowledge=knowledge,
            memory=MemoryRetrievalResult(items=[], reasons={}),
...
def _continuation_task(last_number: int, last_text: str, baseline: dict[str, Any]) -> ContinuationTask:
    milestone = _first_content(baseline, "plot_milestones", "推进主线并延续上一章结果")
    ...
    return ContinuationTask(
        ...
        open_hooks=hooks,
        last_chapter_ending=_compact(last_text, 500),
        character_states=characters[:8],
        min_chinese_chars=max(1000, int(_chinese_char_count(last_text) * 0.8)),
    )
