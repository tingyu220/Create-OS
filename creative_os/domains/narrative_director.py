    ...
    def propose(
        ...
        if projection is not None:
            if proposal.schema_version != 3:
                raise NarrativeDirectorBlockedError("engagement-enabled production requires schema v3")
        ...
        elif proposal.schema_version != 2:
            raise NarrativeDirectorBlockedError("new narrative production without engagement requires schema v2")
        if proposal.schema_version >= 2:
            scene_plan = proposal.chapter_contract.scene_plan
            try:
                scene_plan.validate(required=True)
